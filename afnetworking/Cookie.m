...
static NSString *cookieString = @"cookie";
...
+(void)saveCookie{
    ...
    //存储归档后的cookie
    [[NSUserDefaults standardUserDefaults] setObject:cookieData forKey:cookieString];
...
+(void)takeOutCookie{
    ...
    //对取出的cookie进行反归档处理
    NSArray *cookies = [NSKeyedUnarchiver unarchiveObjectWithData:[userDeefaules objectForKey:cookieString]];
...
+(void)webViewAndCookie:(NSURL *)requestUrl{
    ...
    NSArray *cookies = [NSKeyedUnarchiver unarchiveObjectWithData:[[NSUserDefaults standardUserDefaults] objectForKey:cookieString]];
    NSHTTPCookieStorage *cookieStorage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
